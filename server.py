...
def get_program_links(school_name: str) -> str:
    ...
    if not schools:
        return (
            ...
            f"again with that school name. If no school can be determined, share "
            f"the fallback link above and ask the student which school they mean. "
            f"Do NOT respond with only 'I don't have that information' — always "
            f"give the student somewhere actionable to go."
        )
    ...
    # Multiple schools matched (comparison query)
...
def get_contact_info(school_name: str) -> str:
    ...
    ── WHEN YOUR KNOWLEDGE BASE HAS NO ANSWER ───────────────────────────────────
    CRITICAL: If your knowledge source does not have specific information to answer
    the student's question (e.g., specific deadlines, post-deadline policies,
    MS CS admission requirements, GRE waiver details for a specific program),
    do NOT respond with only "I don't have that information." Instead, call this
    tool with the relevant school name so you can direct the student to the
    school's official contact page. This turns every knowledge gap into a useful,
    actionable response for the student.

    Example: Student asks "What are the application deadlines for Luddy?"
    → Knowledge base has no answer → call get_contact_info("Luddy") and respond:
    "I don't have the specific deadlines in my knowledge base, but you can reach
    the Luddy School directly at: [contact_url]"

    If no specific school can be inferred from context, still call this tool with
    the most likely school, or use get_program_links to surface the IU Graduate
    admissions page as a fallback.
    ...
    if not schools:
        return (
            ...
            f"If no school can be determined, share the fallback link above and "
            f"ask the student which school they are referring to. "
            f"Do NOT respond with only 'I don't have that information' — always "
            f"give the student somewhere actionable to go."
        )
...
if USE_STREAMABLE_HTTP:
    # Streamable HTTP transport (newer; used by some Claude integrations)
    ...
    # SSE transport (default; broadly compatible)
    sse_app = mcp.sse_app()
