...
## What Changed from the Original Servers

This server combines:
- **`iu-grad-assistant-main`** — `get_housing_links` tool, detailed LLM docstrings,
  streamable HTTP support, `FixHostMiddleware`.
- **`mcpserver-firstbranch`** — 15-school data, modular `schools.py` / `matching.py`,
  word-boundary keyword matching, multi-school queries, Starlette SSE app, `/health` endpoint.
